...
/// Shell widget for employee role with bottom navigation
class EmployeeShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final mainNavItems = NavConfig.employeeMainNavItems;
    final moreNavItems = NavConfig.employeeMoreNavItems;

    // Find selected index in main nav items
    // This handles deep-linking correctly (e.g., /e/leave/apply still highlights Leave)
    final selectedIndex = mainNavItems.indexWhere(
      (item) => currentRoute == item.route || currentRoute.startsWith('${item.route}/'),
    );

    // Check if current route is in "More" items (for highlighting when in More sheet routes)
